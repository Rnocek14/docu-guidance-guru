-- ============================================================
-- BLOCKER FIXES: Production Safety Migration (v2)
-- ============================================================

-- 1) Enforce payment_reference when paid (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'payouts_paid_requires_payment_reference'
  ) THEN
...
-- 2) Add updated_at column + trigger to payouts
ALTER TABLE public.payouts
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_payouts_updated_at'
  ) THEN
...
-- 3) Add status index for admin queue queries
CREATE INDEX IF NOT EXISTS idx_payouts_status ON public.payouts(status);

-- ============================================================
-- 4) KYC GATING: Update calculate_payout_eligibility RPC
-- ============================================================
...
-- ============================================================
-- 5) VELOCITY LIMITING: Update validate_payout_request RPC
-- ============================================================
...
BEGIN
  ...
  -- ============================================
  -- VELOCITY LIMIT (BLOCKER FIX)
  -- Max 3 open requests per account in 24 hours
  -- ============================================
