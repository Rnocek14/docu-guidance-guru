-- Fix 4 critical issues in payout RPCs:
-- 1. Timezone-safe date math (America/New_York)
-- 2. Audit logging wrapped in exception handler (won't block flow)
-- 3. Hard-stop returns JSON instead of RAISE EXCEPTION
-- 4. payout_window_opened always included in responses
...
BEGIN
  ...
  -- FIX #1: Timezone-safe date math using America/New_York
...
-- Update validate_payout_request with safe audit logging and JSON return instead of exception
...
BEGIN
  ...
  IF NOT (_eligibility->>'eligible')::boolean THEN
    -- FIX #2: Safe audit logging (wrapped in exception handler)
  ...
  -- FIX #3: Server-side hard stop returns JSON instead of RAISE EXCEPTION
  _payout_window_opened := COALESCE((_eligibility->>'payout_window_opened')::boolean, true);
