...
-- Add reason_code to all return paths in calculate_payout_eligibility
-- This enables cleaner UI logic, analytics, support tooling, and future i18n.
...
BEGIN
  ...
  IF _realized_profit <= 0 THEN
    -- Compute buffer remaining even when profit <= 0 for consistent UX
    _profit_buffer_required := _cohort.min_profit_buffer;
    IF _profit_buffer_required IS NOT NULL AND _has_prior_payout THEN
      _profit_buffer_remaining := _profit_buffer_required; -- full buffer remaining
