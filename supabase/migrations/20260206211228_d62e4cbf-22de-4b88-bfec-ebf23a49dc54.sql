...
-- FIX: NO_PROFIT branch computes accurate buffer remaining (required - realized_profit)
-- instead of just using the full required amount
...
BEGIN
  ...
  IF _realized_profit <= 0 THEN
    ...
    IF _profit_buffer_required IS NOT NULL AND _has_prior_payout THEN
      -- FIX: accurate remaining even when profit is negative
