-- ============================================
-- Per-tier lifetime cap tracking
-- ============================================

-- New table: tracks lifetime_paid_total per user per cohort
CREATE TABLE public.user_cohort_payouts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL,
  cohort_id uuid NOT NULL REFERENCES public.cohorts(id),
  lifetime_paid_total numeric NOT NULL DEFAULT 0 CHECK (lifetime_paid_total >= 0),
...
-- Enable RLS
ALTER TABLE public.user_cohort_payouts ENABLE ROW LEVEL SECURITY;

-- RLS policies
CREATE POLICY "Users can view own cohort payouts"
  ON public.user_cohort_payouts FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Staff can view all cohort payouts"
  ON public.user_cohort_payouts FOR SELECT
  USING (has_any_role(auth.uid(), ARRAY['risk_officer'::app_role, 'support'::app_role, 'admin'::app_role]));

CREATE POLICY "No client inserts on user_cohort_payouts"
  ON public.user_cohort_payouts FOR INSERT
  WITH CHECK (false);

CREATE POLICY "No client updates on user_cohort_payouts"
  ON public.user_cohort_payouts FOR UPDATE
  USING (false);

CREATE POLICY "No client deletes on user_cohort_payouts"
  ON public.user_cohort_payouts FOR DELETE
  USING (false);

-- Trigger for updated_at
CREATE TRIGGER update_user_cohort_payouts_updated_at
  BEFORE UPDATE ON public.user_cohort_payouts
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

-- ============================================
-- Updated calculate_payout_eligibility RPC
-- Now uses per-cohort lifetime tracking
-- ============================================
...
-- ============================================
-- Updated validate_payout_request RPC
-- Now uses per-cohort lifetime tracking
-- ============================================
...
-- ============================================
-- Updated mark_payout_paid RPC
-- All correctness gaps + hardening fixes:
-- 1. Per-cohort lifetime tracking
-- 2. COALESCE for NULL safety
-- 3. NOT FOUND check for missing rows
-- 4. RETURNING for accurate post-update value
-- 5. Minimum payout defense-in-depth
-- 6. Lock account row for concurrent safety
-- ============================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    -- Re-fetch with lock if INSERT returned nothing (concurrent insert)
    IF NOT FOUND THEN
      SELECT * INTO _cohort_payout
      FROM user_cohort_payouts
      WHERE user_id = _account.user_id AND cohort_id = _account.cohort_id
  ...
  IF _cohort.entry_fee IS NOT NULL AND _cohort.lifetime_cap_multiple IS NOT NULL THEN
    ...
    IF _payout_amount > _lifetime_headroom THEN
      RETURN jsonb_build_object(
        ...
        'lifetime_paid_total', _lifetime_paid_total,
        'cohort_id', _account.cohort_id,
        'hint', 'The user has reached or is exceeding their lifetime payout cap for this tier.'
      );
    END IF;
  ELSE
    _lifetime_cap_amount := NULL;
  ...
  -- Atomically increment per-cohort lifetime_paid_total using RETURNING
  UPDATE user_cohort_payouts
  SET lifetime_paid_total = COALESCE(lifetime_paid_total, 0) + _payout_amount
  WHERE user_id = _account.user_id AND cohort_id = _account.cohort_id
  RETURNING lifetime_paid_total INTO _new_lifetime_paid_total;
  
  -- Also update the global profiles.lifetime_paid_total for backward compat / reporting
  UPDATE profiles
  SET lifetime_paid_total = COALESCE(lifetime_paid_total, 0) + _payout_amount
  ...
  RETURN jsonb_build_object(
    ...
    'lifetime', jsonb_build_object(
      'cohort_id', _account.cohort_id,
      'new_lifetime_paid_total', _new_lifetime_paid_total,
      'lifetime_cap_amount', _lifetime_cap_amount
    )
  );
END;
