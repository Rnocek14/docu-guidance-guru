-- ============================================
-- All function updates from previous attempt (without the failing indexes)
-- ============================================

-- UPDATED: calculate_payout_eligibility
...
BEGIN
  ...
  IF _last_payout IS NOT NULL THEN
    ...
    -- FIX: Count trading days using EITHER opened OR closed activity (fairer)
...
-- UPDATED: detect_trade_correlations with lookback + scoring
...
BEGIN
  ...
  _lookback_cutoff := now() - (_lookback_days || ' days')::interval;
  
  -- Opposite-side detection
  FOR _correlation IN
    WITH opposite_matches AS (
      SELECT
        ...
        CASE 
          WHEN ABS(EXTRACT(EPOCH FROM (t2.opened_at - t1.opened_at))) <= 2 THEN 1.0
          WHEN ABS(EXTRACT(EPOCH FROM (t2.opened_at - t1.opened_at))) <= 10 THEN 0.7
          WHEN ABS(EXTRACT(EPOCH FROM (t2.opened_at - t1.opened_at))) <= 30 THEN 0.5
          ELSE 0.3
        END AS timing_weight
      FROM trades t1
      ...
      WHERE t1.account_id = _account_id
        ...
        AND t1.opened_at >= _lookback_cutoff
        AND t2.opened_at >= _lookback_cutoff
    ),
    ranked_matches AS (
      SELECT *, ROW_NUMBER() OVER (PARTITION BY other_account_id ORDER BY our_time DESC) as rn
    ...
    scored AS (
      SELECT
        ...
        ROUND(SUM(timing_weight)::numeric, 2) AS correlation_score,
        jsonb_agg(
          jsonb_build_object(
            'symbol', symbol, 'our_side', our_side, 'their_side', their_side,
            'our_time', our_time, 'their_time', their_time,
            'time_diff_seconds', time_diff_seconds, 'timing_weight', timing_weight
    ...
    SELECT *, 'opposite_side' AS correlation_type FROM scored
  LOOP
    _correlations := _correlations || jsonb_build_object(
      ...
      'correlation_score', _correlation.correlation_score,
  ...
  FOR _correlation IN
    ...
    SELECT *, 'same_side' AS correlation_type FROM scored
  LOOP
    _correlations := _correlations || jsonb_build_object(
      'other_account_id', _correlation.other_account_id,
      'other_account_number', _correlation.other_account_number,
...
-- UPDATED: validate_payout_request with lifecycle + pending checks
...
-- UPDATED: check_payout_method_duplicate with NULL safety
...
BEGIN
  SELECT 
    ...
    (ARRAY_AGG(block_reason) FILTER (WHERE is_blocked = true))[1]
  INTO _existing_count, _blocked, _block_reason
  FROM payout_methods
  WHERE method_hash = _method_hash AND user_id != _user_id;
  
  IF _blocked THEN
    RETURN jsonb_build_object(
      'allowed', false,
      'reason', COALESCE(_block_reason, 'This payout method has been blocked'),
      'is_blocked', true,
...
-- Standard indexes (no time-based predicates)
CREATE INDEX IF NOT EXISTS idx_trades_correlation_lookup 
ON trades (account_id, symbol, opened_at DESC);

CREATE INDEX IF NOT EXISTS idx_payouts_pending_by_account
ON payouts (account_id) WHERE status IN ('pending', 'under_review', 'approved');