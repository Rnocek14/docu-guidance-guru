...
DECLARE
  ...
  _winning_days_since_payout integer;
  ...
  _required_winning_days integer;
BEGIN
  ...
  _required_winning_days := _cohort.min_trading_days_between_payouts;
  ...
  IF _last_payout IS NOT NULL THEN
    ...
    -- Count winning days (net-positive PnL days) since last paid payout
    WITH day_pnl AS (
      SELECT
    ...
    IF _winning_days_since_payout < _required_winning_days THEN
      RETURN jsonb_build_object(
        'eligible', false, 'reason', 'Not enough winning trading days since last payout',
        ...
        'payout_window_opened', true,
        'hint', 'You need ' || _winning_days_remaining || ' more winning trading day(s) before your next payout'
      );
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'days_since_last_payout', _days_since_last_payout,
    'winning_days_since_payout', COALESCE(_winning_days_since_payout, 0),
    'required_winning_days', _required_winning_days,
    'winning_days_remaining', COALESCE(_winning_days_remaining, 0),
    'winning_days_progress_pct', ROUND(COALESCE(_winning_days_progress_pct,
      CASE
        WHEN COALESCE(_required_winning_days, 0) <= 0 THEN 100
        ELSE LEAST(100, GREATEST(0, (COALESCE(_winning_days_since_payout, 0)::numeric / _required_winning_days) * 100))
      END
    ), 1),
