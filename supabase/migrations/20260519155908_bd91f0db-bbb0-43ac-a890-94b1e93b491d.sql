-- BREAKER v1.1: Pay/Rev signal — retry with DROP for return-type change

ALTER TYPE public.audit_action ADD VALUE IF NOT EXISTS 'breaker_transition';

ALTER TABLE public.econ_breaker_state
  ADD COLUMN IF NOT EXISTS rolling_payrev_ratio numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS payrev_revenue_30d numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS payrev_payouts_30d numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS payrev_window_days integer NOT NULL DEFAULT 30,
  ADD COLUMN IF NOT EXISTS payrev_level text NOT NULL DEFAULT 'normal',
  ADD COLUMN IF NOT EXISTS payrev_release_streak integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last_transition_at timestamptz NULL,
  ADD COLUMN IF NOT EXISTS last_transition_reason text NULL;
...
BEGIN
  ...
  v_payrev := CASE WHEN v_revenue_30d > 0 THEN v_payouts_30d / v_revenue_30d ELSE 0 END;
  ...
  v_reason := format(
    'pass=%s%% [%s] | payrev=%s%% [%s, streak=%s] | buffer=$%s [%s]',
    round(v_pass_rate, 2), v_pass_level,
    round(v_payrev * 100, 2), v_new_payrev_level, v_new_streak,
  ...
  UPDATE econ_breaker_state SET
    ...
    payrev_revenue_30d = v_revenue_30d,
    ...
    BEGIN
      ...
      VALUES (
        ...
        jsonb_build_object(
          ...
          'revenue_30d', round(v_revenue_30d, 2),
...
AS $$
  SELECT
    ...
    rolling_payrev_ratio, payrev_revenue_30d, payrev_payouts_30d,
    payrev_window_days, payrev_level, payrev_release_streak,
