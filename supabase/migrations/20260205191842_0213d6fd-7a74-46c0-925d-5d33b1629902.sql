-- Drop all existing overloaded versions of get_liability_snapshot
DROP FUNCTION IF EXISTS public.get_liability_snapshot(integer);
DROP FUNCTION IF EXISTS public.get_liability_snapshot(integer, numeric, numeric);

-- Recreate with consistent signature: days_forward, cash_reserve, assumed_avg_first_payout
...
BEGIN
  ...
  -- Input clamping (defense in depth)
  ...
  -- Net buffer calculation with safe parsing (NULLIF handles empty strings)
