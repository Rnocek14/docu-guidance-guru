...
DECLARE
  ...
  -- Minimum expected runs per job in 65-min window (5-min cron = ~13 runs)
...
BEGIN
  ...
  v_success_rate := CASE WHEN v_total_runs > 0
    THEN round((v_success_count::numeric / v_total_runs) * 100, 1)
    ELSE 0 END;

  -- 2) Missing jobs (zero runs in window)
  ...
  -- 3) Jobs with too few runs (degraded but not missing)
  SELECT array_agg(j) INTO v_low_run_jobs
  FROM unnest(v_expected_jobs) AS j
  WHERE j != ALL(COALESCE(v_missing_jobs, '{}'))
  ...
  -- 5) Last 3 statuses (recent history snapshot)
  ...
  FROM (
    ...
    FROM cron_http_runs
    WHERE jobname = ANY(v_expected_jobs)
  ...
  -- 7) Evaluate health — all clear = exit
  IF v_failed_count = 0
  ...
  -- 8) Build issue list for title
  IF COALESCE(array_length(v_missing_jobs, 1), 0) > 0 THEN
    v_issues := v_issues || 'missing jobs';
  END IF;
  IF v_failed_count > 0 THEN
    v_issues := v_issues || (v_failed_count || ' failures');
  END IF;
  IF COALESCE(array_length(v_low_run_jobs, 1), 0) > 0 THEN
    v_issues := v_issues || 'low run count';
  END IF;

  -- Severity: missing jobs or >50% failure = red, else yellow
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'min_expected_runs', v_min_expected_runs,
      'missing_jobs', v_missing_jobs,
      'low_run_jobs', v_low_run_jobs,
      'last_success', v_last_success,
      'last_3_statuses', v_last_3_statuses,
