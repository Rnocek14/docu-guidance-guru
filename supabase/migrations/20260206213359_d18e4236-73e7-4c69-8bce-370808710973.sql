...
-- Add profit_buffer_progress_pct to all return paths for consistent UI rendering
...
BEGIN
  ...
  IF _realized_profit <= 0 THEN
    ...
    IF _profit_buffer_required IS NOT NULL AND _has_prior_payout THEN
      ...
      _profit_buffer_progress_pct := LEAST(100, GREATEST(0, (_realized_profit / _profit_buffer_required) * 100));
    END IF;
    RETURN jsonb_build_object(
