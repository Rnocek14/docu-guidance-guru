...
-- ============================================================
-- PROFIT BUFFER GATE: Repeat payouts require minimum profit accumulation
-- Policy: After first payout, trader must accumulate >= min_profit_buffer
-- in realized profit since last payout cycle reset before requesting another.
-- ============================================================

-- 1) Add min_profit_buffer to cohorts (nullable = disabled)
ALTER TABLE public.cohorts
ADD COLUMN IF NOT EXISTS min_profit_buffer numeric DEFAULT NULL;

COMMENT ON COLUMN public.cohorts.min_profit_buffer IS
  'Minimum realized profit required since last payout before requesting another. NULL = disabled. Recommended: 300-500.';

-- 2) Update calculate_payout_eligibility to enforce profit buffer gate
...
BEGIN
  ...
  -- ============================================================
  -- PROFIT BUFFER GATE (repeat payouts only)
  -- After first payout, trader must accumulate >= min_profit_buffer
  -- in realized profit before requesting another payout.
  -- _realized_profit IS "profit since last payout" because
  -- reset_payout_cycle resets the baseline on each confirmed payout.
  -- ============================================================
  _profit_buffer_required := _cohort.min_profit_buffer;
  
  IF _profit_buffer_required IS NOT NULL AND NOT _is_first_payout_in_cycle THEN
    ...
    IF NOT _profit_buffer_met THEN
      RETURN jsonb_build_object(
        'eligible', false,
        'reason', 'Profit buffer not yet met since last payout',
  ...
  RETURN jsonb_build_object(
    ...
    -- Profit buffer fields (always returned for UI)
    'profit_buffer_required', _profit_buffer_required,
...
ALTER FUNCTION public.calculate_payout_eligibility OWNER TO postgres;
REVOKE EXECUTE ON FUNCTION public.calculate_payout_eligibility FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.calculate_payout_eligibility TO authenticated;
GRANT EXECUTE ON FUNCTION public.calculate_payout_eligibility TO service_role;
