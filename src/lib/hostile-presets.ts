...
export const HOSTILE_PRESETS: HostilePreset[] = [
  ...
  // ── Gate sweep: 3-point bracket to find optimal engage threshold ──
  {
    presetId: 'gate-sweep-036',
    name: 'Gate Sweep: Engage @ 0.36',
    description: 'Conditional pacing with engage threshold at 36% pay/rev ratio. Just above normal range — should cut deferrals vs 0.33 without missing hot months.',
    scenarioVersion: 'v1.4',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
      fixedMonthlyCosts: 12000,
      ...
      payRevEngageThreshold: 0.36,
    },
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with conditional pacing' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.45, description: 'Pay/Rev P95 < 45%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.15, description: 'Deferral rate < 15%' },
      { type: 'MARGIN_ABOVE', threshold: 0.20, description: 'Margin stays above 20%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },
  {
    presetId: 'gate-sweep-038',
    name: 'Gate Sweep: Engage @ 0.38',
    description: 'Conditional pacing with engage threshold at 38% pay/rev ratio. Should only clamp true spikes — likely lowest deferral while keeping tail control.',
    scenarioVersion: 'v1.4',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with conditional pacing' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.45, description: 'Pay/Rev P95 < 45%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.12, description: 'Deferral rate < 12%' },
      { type: 'MARGIN_ABOVE', threshold: 0.20, description: 'Margin stays above 20%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },
  {
    presetId: 'gate-sweep-040',
    name: 'Gate Sweep: Engage @ 0.40',
    description: 'Engage threshold equals budget cap — budget only fires on months already exceeding the target. Near-zero baseline deferral but weaker tail control.',
    scenarioVersion: 'v1.4',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
      ...
      reserveThreshold: 15000,
      targetPayRevSoft: 0.40,
      payRevEngageThreshold: 0.40,
    },
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with conditional pacing' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.50, description: 'Pay/Rev P95 < 50% (weaker control expected)' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.08, description: 'Deferral rate < 8% (near-zero)' },
      { type: 'MARGIN_ABOVE', threshold: 0.18, description: 'Margin stays above 18%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },

  // ── Clustered stress test at winning gate (run after picking winner) ──
  {
    presetId: 'gate-sweep-clustered',
    name: 'Gate Sweep: Winner + Clustering',
    description: 'Best gate from sweep under 0.5 attack intensity. Uses 0.38 as starting point — adjust after sweep results.',
    scenarioVersion: 'v1.4',
    severity: 'critical',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      ...
      { type: 'MARGIN_ABOVE', threshold: 0.15, description: 'Margin stays above 15%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.30, description: 'Deferral rate < 30% under clustering (informational)', isInformational: true },
    ],
  },
];
