...
export const HOSTILE_PRESETS: HostilePreset[] = [
  ...
  // ── Budget sweep: 3-point bracket to find optimal soft budget cap ──
  {
    presetId: 'budget-sweep-040',
    name: 'Budget Sweep: Cap @ 40%',
    description: 'Baseline budget cap at 40% pay/rev. Gate at 0.38. Establishes the structural deferral cost of the current cap.',
    scenarioVersion: 'v1.5',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
      fixedMonthlyCosts: 12000,
    ...
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with 40% budget' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.45, description: 'Pay/Rev P95 < 45%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.25, description: 'Deferral rate < 25% (reference point)' },
      { type: 'MARGIN_ABOVE', threshold: 0.20, description: 'Margin stays above 20%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },
  {
    presetId: 'budget-sweep-045',
    name: 'Budget Sweep: Cap @ 45%',
    description: 'Raised budget cap to 45% pay/rev. Gate at 0.38. Expected to materially reduce deferrals while keeping tail under control.',
    scenarioVersion: 'v1.5',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with 45% budget' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.50, description: 'Pay/Rev P95 < 50%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.15, description: 'Deferral rate < 15% (launch target)' },
      { type: 'MARGIN_ABOVE', threshold: 0.18, description: 'Margin stays above 18%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },
  {
    presetId: 'budget-sweep-050',
    name: 'Budget Sweep: Cap @ 50%',
    description: 'Aggressive budget cap at 50% pay/rev. Gate at 0.38. Low deferral but hotter tails — tests if breaker backstop is sufficient.',
    scenarioVersion: 'v1.5',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
      ...
      iterations: 2000,
      reserveThreshold: 15000,
      targetPayRevSoft: 0.50,
      payRevEngageThreshold: 0.38,
    },
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with 50% budget' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.55, description: 'Pay/Rev P95 < 55%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.10, description: 'Deferral rate < 10% (aggressive target)' },
      { type: 'MARGIN_ABOVE', threshold: 0.15, description: 'Margin stays above 15%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.08, description: 'Reserve breach < 8%' },
    ],
  },

  // ── Clustered stress test at likely winner (0.45 budget) ──
  {
    presetId: 'budget-sweep-clustered',
    name: 'Budget Sweep: 45% + Clustering',
    description: '45% budget cap under 0.5 attack intensity. Validates that raised budget holds under adversarial payout bunching.',
    scenarioVersion: 'v1.5',
    severity: 'critical',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with 45% budget + clustering' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.55, description: 'Pay/Rev P95 < 55% under clustering' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.15, description: 'Reserve breach < 15%' },
      { type: 'WORST_MONTH_ABOVE', threshold: -15000, description: 'Worst month > -$15k' },
      { type: 'MARGIN_ABOVE', threshold: 0.15, description: 'Margin stays above 15%' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.35, description: 'Deferral rate < 35% under clustering (informational)', isInformational: true },
    ],
  },
];
