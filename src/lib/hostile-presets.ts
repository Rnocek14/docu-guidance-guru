...
export const HOSTILE_PRESETS: HostilePreset[] = [
...

  {
    ...
    name: 'Pacing: Conditional 40% Budget',
    description: 'Conditional payout budgeting (engage at 33% pay/rev, cap at 40%). Budget only activates in hot months — normal months flow freely.',
    scenarioVersion: 'v1.3',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
      fixedMonthlyCosts: 12000,
    ...
    expectedAssertions: [
      ...
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.15, description: 'Deferral rate < 15% (most payouts flow normally)' },
      { type: 'MARGIN_ABOVE', threshold: 0.20, description: 'Margin stays above 20%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.05, description: 'Reserve breach < 5%' },
    ],
  },
  {
    ...
    name: 'Pacing: Conditional 35% Budget (Aggressive)',
    description: 'Tighter conditional budget (engage at 30%, cap at 35%). Tests if aggressive clamp keeps deferrals reasonable.',
    scenarioVersion: 'v1.3',
    severity: 'warning',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      ...
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.20, description: 'Deferral rate < 20% (UX ceiling)' },
      { type: 'MARGIN_ABOVE', threshold: 0.22, description: 'Margin stays above 22%' },
    ],
  },
  {
    presetId: 'pacing-clustered-040',
    name: 'Pacing: Clustering + Conditional 40% Budget',
    description: 'Conditional payout budgeting under correlated payout timing. Tests whether conditional pacing absorbs spikes without over-deferring.',
    scenarioVersion: 'v1.3',
    severity: 'critical',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      ...
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.25, description: 'Deferral rate < 25% under clustering (informational)', isInformational: true },
    ],
  },
  {
    presetId: 'pacing-excitement-040',
    name: 'Pacing: 85% Split + $750 Cap + Conditional 40% Budget',
    description: 'Full excitement stack WITH conditional payout budgeting. Tests whether conditional pacing makes excitement levers launch-safe.',
    scenarioVersion: 'v1.3',
    severity: 'critical',
    inputs: {
      accountsPerMonth: 200,
    ...
    expectedAssertions: [
      { type: 'ANNUAL_PROFIT_POSITIVE', description: 'Profitable with excitement + conditional pacing' },
      { type: 'PAY_REV_P95_BELOW', threshold: 0.45, description: 'Pay/Rev P95 < 45% (excitement controlled by budget)' },
      { type: 'DEFERRAL_RATE_BELOW', threshold: 0.18, description: 'Deferral rate < 18%' },
      { type: 'MARGIN_ABOVE', threshold: 0.15, description: 'Margin stays above 15%' },
      { type: 'RESERVE_BREACH_BELOW', threshold: 0.10, description: 'Reserve breach < 10%' },
    ],
  },
